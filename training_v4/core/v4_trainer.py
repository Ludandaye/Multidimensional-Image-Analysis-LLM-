...
class V4Trainer:
    ...
    def _load_and_prepare_data(self):
            ...
            for path in vocab_paths:
                try:
                    with open(path, 'r', encoding='utf-8') as f:
                        vocab = json.load(f)
                    self.logger.info(f"📚 从 {path} 加载词汇表")
                    break
        ...
        # 分层数据划分：确保每个类别在训练/验证集均有覆盖
        rng = random.Random(42)
        label_to_items: Dict[int, List[Dict[str, Any]]] = {}
        for item in processed_data:
            label = int(item['cls_label'])
        ...
        for label, items in label_to_items.items():
            ...
            n_train = min(n_train, n - 1) if n > 1 else 1  # 保证验证集至少1个样本（当n>1）
